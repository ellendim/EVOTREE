...
type = "NorWood"
expr_file = "NorWood_transcriptomics.txt.gz"
...
samples <- c("Genes", paste0("S3_", 1:27),
             c("S2_1", "S2_2_1", "S2_2_2", "S2_2_3", paste0("S2_", 3:23)),
             paste0("S1_", 1:28))

# Counts
counts <- read_tsv("../DATA/Counts/NorWood_salmon.merged.gene_counts.tsv") %>% 
  select(-gene_name) %>% 
...
# Rename samples
data.frame(Samples = samples[2:length(samples)]) %>% 
  mutate(Samples = gsub("S3_", "N1.", Samples)) %>%
  mutate(Samples = gsub("S2_", "N2.", Samples)) %>% 
  mutate(Samples = gsub("S1_", "N3.", Samples)) %>% 
  mutate(Samples = gsub("N2.1$", "N2.-1", Samples)) %>% 
  mutate(Samples = gsub("N2.2_1", "N2.-2", Samples)) %>%
  mutate(Samples = gsub("N2.2_2", "N2.-3", Samples)) %>%
  mutate(Samples = gsub("N2.2_3", "N2.-4", Samples)) %>%
  separate(Samples, into = c("Tree", "Section"), sep = "\\.") %>% 
  mutate(Section = as.numeric(Section)) %>% 
  mutate(Section = ifelse(Tree == "N2" & Section >= 3, Section+2, Section)) %>% 
  mutate(Section = abs(Section)) %>% 
  mutate(Samples = paste(Tree, Section, sep = ".")) %>% 
...
# Remove TEs
TEs <- read_tsv("../DATA/Annotations/SPRUCE_TE_IDs_REMOVED.txt", col_names = FALSE) %>%
...
# Remove outliers
remove_samples <- c("N2.25", "N3.1", "N3.28", # Low depth/Few genes
                    "N2.7", "N2.8", "N2.9", "N2.10", "N2.16", "N2.17", "N2.18",
                    "N3.26", "N3.27", # Outliers 1
                    "N2.4", "N2.5", "N2.6" # Outliers 2
                    )
...
cat(length(expressed_genes), "expressed genes with two samples with VST > 3 in two trees\n")
...
# Cluster samples
expr.wide.N1 <- expr.wide %>% 
  select(contains("N1"))
n.N1 <- ncol(expr.wide.N1)
cdist <- dist(t(expr.wide.N1), method="euclidean")
#cdist <- as.dist(1-cor(expr.wide.N1, method="pearson"))
chc <- hclust(cdist, method="ward.D2")
cdendro <- as.dendrogram(chc)

order <- rep(0, n.N1)
h <- colnames(expr.wide.N1)
...
colgenes <- c("mediumpurple3","#CC5151", "#B26F2C", "lightsteelblue3","#A3CC51", "#51A3CC")
colgenes <- c("#CC5151", "mediumpurple3","#B26F2C", "black", "#A3CC51", "lightsteelblue3", "#51A3CC")
...
h <- heatmap.2(as.matrix(expr.wide.N1),
        Rowv = rdendro,
        Colv = cdendro,
        colsep = cbreaks,
        sepcolor = "black",
...
for (i in 1:rnclust) {
  ...
  plots[[i]] <- cluster %>%
    ...
    geom_line(na.rm = T) + ylim(range(centroid$Expression.scaled)) +
...
p <- cbind(pc$x, data.frame(Clusters = as.factor(c(cct, rep("black", n-n.N1))),
...
p1 <- ggplot(p, aes(PC1, PC2, col = Clusters)) + 
  ...
  geom_point() +
  scale_color_manual(values=c(colsamples, "black")) +
  theme(legend.position="none")
...
p2 <- ggplot(p, aes(PC1, PC2, col = Clusters)) + 
  ...
  ylab(paste0("PC2 (", round(var.expl[2], digits=2),")")) +
  geom_point() +
  geom_label_repel(aes(label = Samples), box.padding=0.01, point.padding=0.01, 
                   segment.color='grey50', label.size=0.1, size=2.5) +
  scale_color_manual(values=c(colsamples, "black")) +
  theme(legend.position="none")
...
orthogroups <- read_tsv("../DATA/Orthogroups/Orthogroups_20240823_clean.tsv.gz", show_col_types = FALSE) %>%
  select(OrthoGroup, Arabidopsis_thaliana, Picea_abies) %>%
  rowwise(OrthoGroup) %>%
  summarise(Genes = paste(Arabidopsis_thaliana, Picea_abies, sep = ", ")) %>%
  select(OrthoGroup, Genes) %>%
  separate_rows(Genes, sep = ", ") %>%
  filter(Genes != "") %>%
  rowwise() %>%
  mutate(Species = if_else(grepl("^AT", Genes), "Arab", "Nor")) %>%
...
for (i in 1:length(marker_genes_names)) {
  ...
  ids <- orthogroups %>% filter(OrthoGroup == orth, Species == "Nor") %>% pull(Genes)
...
marker_genes_selected[["SUS6"]] <- c("PA_chr10_G000052")
marker_genes_selected[["CDC2"]] <- c("PA_chr04_G001371")
marker_genes_selected[["EXPA1"]] <- c("PA_chr03_G007209")
marker_genes_selected[["CesA8"]] <- c("PA_chr10_G000911")
marker_genes_selected[["BFN1"]] <- c("PA_chr02_G005278")
