...
type = "CherWood"
expr_file = "CherWood_transcriptomics.txt.gz"
...
samples <- c("Genes", 
             paste0("C1-", sprintf("%02d", 1:27)),
             paste0("C2-", sprintf("%02d", 1:27)),
             paste0("C3-", sprintf("%02d", 1:28)))

# Counts
counts <- read_tsv("../DATA/Counts/CherWood_salmon.merged.gene_counts.tsv")
counts <- counts[, samples]

# Rename samples
samples <- gsub("C1-", "C1.", colnames(counts))
samples <- gsub("C2-", "C2.", samples)
samples <- gsub("C3-", "C3.", samples)
...
colgenes <- c("#CC5151", "#51A3CC", "#A3CC51", "lightsteelblue3", "black", "#CC5151", "#B26F2C", "orange")
...
orthogroups <- read_tsv("../DATA/OrthoGroups/Orthogroups_20240823_clean.tsv.gz", show_col_types = FALSE) %>%
  select(OrthoGroup, Arabidopsis_thaliana, Prunus_avium) %>%
  rowwise(OrthoGroup) %>%
  summarise(Genes = paste(Arabidopsis_thaliana, Prunus_avium, sep = ", ")) %>%
  select(OrthoGroup, Genes) %>%
  separate_rows(Genes, sep = ", ") %>%
  filter(Genes != "") %>%
  rowwise() %>%
  mutate(Species = if_else(grepl("^AT", Genes), "Arab", "Cher")) %>%
...
for (i in 1:length(marker_genes_names)) {
  ...
  ids <- orthogroups %>% filter(OrthoGroup == orth, Species == "Cher") %>% pull(Genes)
...
marker_genes_selected[["SUS6"]] <- c("FUN_026680-T1")
marker_genes_selected[["CDC2"]] <- c("FUN_024221-T1")
marker_genes_selected[["EXPA1"]] <- c("FUN_019120-T1")
marker_genes_selected[["CesA8"]] <- c("FUN_030863-T1")
marker_genes_selected[["BFN1"]] <- c("FUN_031924-T1")
