...
VER <- "V5" # first run: "" second run: "-V2"
...
# col_function_1 <- colorRampPalette(brewer.pal(n = 8, "YlOrRd"))(30)
# col_function_1 <- colorRamp2(c(lowest,  between, highest), c("#800026", "#fd8d3c", "#ffffcc"))

corr_df <- melt(get(matrix))
corr_df$Var2 <- factor(corr_df$Var2, levels = rev(unique(corr_df$Var2)))

hm <- ggplot(corr_df, aes(x =Var1, y = Var2, fill = value, colour = value)) + geom_tile(show.legend = F) + xlab(" ") + ylab(" ") + scale_fill_distiller(palette = "YlOrBr")+scale_colour_distiller(palette = "YlOrBr")+
 theme(axis.text=element_blank(), axis.ticks = element_blank())  +scale_x_discrete(expand=c(0,0))+
  scale_y_discrete(expand=c(0,0))

    new_name <- as.character(paste0(key_word_s1, "_",key_word_s2))
...
# 
pdf("hm_asp_cher.pdf", height = 5, width = 5)
Cher_Asp
dev.off()
...
hm <-plot_grid(
          Asp_Asp,NULL, NULL, NULL, NULL,NULL,
          Birch_Asp, Birch_Birch, NULL, NULL,NULL, NULL,
          Cher_Asp,Cher_Birch , Cher_Cher, NULL, NULL, NULL,
          Nor_Asp, Nor_Birch, Nor_Cher, Nor_Nor, NULL, NULL,
          Scots_Asp, Scots_Birch, Scots_Cher, Scots_Nor, Scots_Scots, NULL,
          Lodge_Asp, Lodge_Birch, Lodge_Cher, Lodge_Nor, Lodge_Scots, Lodge_Lodge, nrow = 6, ncol = 6)

 pdf("hm_palette_9.pdf", height=10, width = 10)
 hm
 dev.off()
...
pdf("density_plots.pdf", height =10, width = 5)
p1/p2/p3
...
tree_1 <-ggtree(tree_grouped, aes(color = group),ladderize = F, size = 1) +
  scale_color_manual(values=c( "#66C2A5","#FC8D62", "grey")) + geom_tiplab(color = "grey9",size = 5, fontface = "italic",vjust = -2, hjust = 0.5) + xlim(-10, 10) + theme(legend.position = "none")
tree_1

tree_horo <- tree_1 + rotate()

tree_grouped_vert <- groupClade(tree_full_name_lat, .node = c(8))
tree_vert <- ggtree(tree_grouped_vert, aes(color = group), ladderize = F, size = 1) + geom_tiplab(color = "grey9",size = 5, angle = 0, fontface = "bold.italic", hjust = -0.1)  + xlim(-10, 10) +scale_color_manual(values=c( "#FC8D62","#66C2A5", "grey")) + theme(legend.position = "none")

pdf("vertical_tree.pdf", height=8, width = 4)
tree_vert
dev.off()

pdf("horizontal_tree.pdf", height=4, width = 8)
tree_horo
dev.off()
...
col_fun <- colorRamp2(c(0, 0.5, 1), c("#B10026", "#FD8D3C", "#FFFFCC"))
...
pdf("legend_corr.pdf", height=5, width=5)
grid.rect()
draw(lgd_horizontal)
dev.off()
...
# hm_fig <-image_read("C:/Users/ellen/OneDrive - Norwegian University of Life Sciences/EvoTree_figures_and_txt/Figures_draft/SampleComparison/palette9.png")
hm_fig <-image_read_pdf("hm_palette_9.pdf")
