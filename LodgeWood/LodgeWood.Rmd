...
type = "LodgeWood"
expr_file = "LodgeWood_transcriptomics.txt.gz"
...
samples <- c("Genes", paste0("LP2_", 1:28),
             paste0("LP3_", 1:28),
             paste0("LP5_", 1:28))

# Counts
counts <- read_tsv("../DATA/Counts/LodgeWood_salmon.merged.gene_counts.tsv") %>% 
  select(-gene_name) %>% 
...
# Rename samples
samples <- gsub("LP2_", "L1.", colnames(counts))
samples <- gsub("LP3_", "L2.", samples)
samples <- gsub("LP5_", "L3.", samples)
...
colgenes <- c("#CC5151", "#CC5151", "mediumpurple3", "#B26F2C", "#A3CC51", "orange", "#51A3CC")
